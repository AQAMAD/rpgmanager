...
import com.delegreg.rpgm.ui.widgets.RpgmWebBrowser;

public class HttpDocView extends ViewPart {
	private final FormToolkit formToolkit = new FormToolkit(Display
			.getDefault());
	public static final String ID = "com.delegreg.rpgm.views.http"; //$NON-NLS-1$
	private RpgmWebBrowser browser;
	...
	public void createPartControl(Composite parent) {
		{
			...
			{
				browser = new RpgmWebBrowser(form.getBody(), SWT.NONE);
	...
	private void makeActions() {
		actionIndex = new Action(Messages.HttpDocView_Index, AbstractUIPlugin
			...
			public void run() {
				...
				if (prevnumindexes<newnumindexes){
					MessageDialog.openInformation(getSite().getShell(), Messages.HttpDocView_IndexDialogTitle, Integer.toString(newnumindexes-prevnumindexes) + Messages.HttpDocView_IndexDialogMessage);
	...
	private void openCurrentDoc(){
		if (originURL instanceof HttpDocument){
			...
			setPartName(Messages.HttpDocView_PartName + doc.getName());
		}
