...
public class SequenceEditorLocationsPage extends FormPage {
	public static final String ID = "com.delegreg.rpgm.editors.sequence.locations"; //$NON-NLS-1$
	...
	public SequenceEditorLocationsPage(FormEditor editor, String id, String title) {
		super(editor, id, title);
		this.editor=(SequenceEditor)editor;
	}

	public SequenceEditorLocationsPage(FormEditor editor) {
		this(editor, ID, "Locations");
	...
	protected void createFormContent(IManagedForm managedForm) {
		...
		section.setText("Locations for this sequence"); //$NON-NLS-1$
		section.setDescription("Manage the locations for this sequence, this way you can quickly access them."); //$NON-NLS-1$
		...
		Button b = toolkit.createButton(client, "Add location" , SWT.PUSH); //$NON-NLS-1$
		b.setToolTipText("Add a location to this sequence");
		b.setImage(AbstractUIPlugin.imageDescriptorFromPlugin(Application.PLUGIN_ID, IImageKeys.LOCATION).createImage());
		//b.setData(sb);
			...
			public void widgetSelected(SelectionEvent e) {
				...
				dialog.addFilter(new ViewerFilter(){
					...
					public boolean select(Viewer viewer, Object parentElement,
						...
						element instanceof Groups  || element instanceof Locations || element instanceof Location
						;
					}});
				dialog.setValidator(new ISelectionStatusValidator(){
					...
					public IStatus validate(Object[] selection) {
						for (int i = 0; i < selection.length; i++) {
							if (!(selection[i] instanceof Location)){
				...
				dialog.setTitle("Select a location to add");
				dialog.setDoubleClickSelects(true);
				dialog.setMessage("Select a location to add to this sequence");
				dialog.open();
				//run the runnable to update ressources
				Location l=(Location)dialog.getFirstResult();
				sequ.getLocations().addlink(l);
				viewer.refresh();
			}
			@Override
		...
		b2.addSelectionListener(new SelectionListener() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				Locations loc=sequ.getLocations();
		...
		viewer.setInput(sequ.getLocations());
